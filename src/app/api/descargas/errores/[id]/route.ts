...
export async function GET(
  ...
  try {
    ...
    if (!presentacion.ruta_archivo_errores) {
      return NextResponse.json({ error: 'No hay archivo de errores disponible' }, { status: 404 });
    }

    // Descargar archivo
    const { data, error } = await supabase.storage
      .from('archivos-errores')
      .download(presentacion.ruta_archivo_errores);
    ...
    return new NextResponse(buffer, {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="errores_${presentacion.periodo}.xlsx"`,
      },
    });
  } catch (error) {
